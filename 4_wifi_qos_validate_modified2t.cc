...
int main(int argc, char* argv[])
{
    ...
    for (uint32_t i = 0; i < nSta; i++)
    {
       ...
       if (useML) {
            // 1. Fetch all predictions from the Python ML pipeline
            ...
            // 3. IGNORE out[1] (ru) and out[3] (mcs). 
            // Implement strict mathematical assignment based on Priority.
            switch (c.priority) {
                case 3: 
                    // VoIP: Needs absolute lowest latency, tiny payload.
                    // Assign smallest 26-tone RU (RU=1) so it doesn't hog the channel.
                    c.ru = 1;  
                    c.mcs = 4; // Robust modulation for reliable voice
                    break;
                case 2: 
                    // Video: Needs high throughput and low jitter.
                    // Assign large 40MHz equivalent (RU=18)
                    c.ru = 18; 
                    c.mcs = 9; // High QAM modulation for high data rate
                    break;
                case 1: 
                    // Best Effort (HTTP / VPN): Standard traffic.
                    // Assign medium 20MHz equivalent (RU=9)
                    c.ru = 9;  
                    c.mcs = 6; // Standard modulation
                    break;
                case 0: 
                default:
                    // Background / IoT: Infrequent, power-saving, small payload.
                    // Assign smallest 26-tone RU (RU=1) 
                    c.ru = 1;  
                    c.mcs = 0; // Most robust (BPSK) for maximum range/reliability
                    break;
            }
        } else {
        ...
        double minIntMs  = (c.pktSize * 8.0)
                           / (ruMaxMbps * 1e6) * 1000.0;
        c.effIntMs = std::max(c.intervalMs,
                              (int)std::ceil(minIntMs));

        // TWT controls when station starts transmitting
        // Higher priority (lower TWT) → starts sooner
        c.startTime    = 2.0
                       + (double)c.twt / 1000.0
                       + i * 0.1;
